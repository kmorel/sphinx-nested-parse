  ...
  Here is a simple example::
    ...
    class Foo(sphinx.util.docutils.SphinxDirective):
      def run(self):
        ...
        parse.add_line('.. note::', 'fakefile.rst', 1)
        parse.add_line('   This box added in directive.', 'fakefile.rst', 2)
        parse.add_line('', 'fakefile.rst', 3)
        parse.add_line('This text also added in directive.', 'fakefile.rst', 3)
        return parse.get_nodes(self)

    def setup(app):
      app.add_directive('foo', Foo)

  Note that if you want the nested parse to base its file location on the
  location of your directive (which is a good idea), you can get the name
  of the file containing the directive call with
  ``self.state.document.current_source`` and get the line to directive
  starts at with ``self.lineno``.
